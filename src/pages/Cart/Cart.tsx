...
import {getPizzasCartItems, getTotalCount, getTotalPrice} from "../../redux/selectors/cart";
...
const Cart = () => {
    ...
    const pizzasCartItems = useSelector(getPizzasCartItems);

    useEffect(() => {
        dispatch(actions.setIsShowButtonToCart(false));
        return () => {
            dispatch(actions.setIsShowButtonToCart(true));
        };
    }, [])

    const handlerClickClearItem = () => {
        ...
        return (
            <div className={style.cart}>
                <section className={style.topContent}>
                    ...
                    <div className={style.clearItem} onClick={handlerClickClearItem}>
                ...
                <section className={style.mainContent}>
                    {Object.keys(pizzasCartItems).map((key, index) => <CartItem pizzaCartItems={pizzasCartItems[Number(key)]} key={key+index}/>)}
                </section>
                <section className={style.bottomContent}>
                    <div className={style.info}>
                        <div className={style.pizzaCount}>Всего пицц: <span>{totalCount} шт.</span></div>
                        <div className={style.pizzaTotalPrice}>Сумма заказа: <span>{totalPrice} ₽</span></div>
                    </div>
                    <div className={style.buttons}>
                        <Button className="button--back">
                            <NavLink to={"/"}>
                                <svg width="8" height="14" viewBox="0 0 8 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M7 13L1 6.93015L6.86175 1" fill="#FFFFFF" stroke="#D3D3D3" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                                </svg>
